...
export default function Footer() {
    return (
        <footer className="flex flex-wrap justify-center md:justify-between overflow-hidden gap-10 md:gap-20 mt-40 py-6 px-6 md:px-16 lg:px-24 xl:px-32 text-[13px] text-gray-500">
            ...
            >
                <a href="/">
                    <img className="size-8 aspect-square" src="/logo.svg" alt="Snappify logo" width={32} height={32} />
                </a>
            ...
            <motion.div className="flex flex-col max-md:items-center max-md:text-center gap-2 items-end"
                ...
                transition={{ type: "spring", stiffness: 280, damping: 70, mass: 1 }}
            >
                <p className="max-w-60">AI-powered short ads that convert — built for founders, marketers, and creators.</p>
                <div className="flex items-center gap-4 mt-3">
                    <a href="https://instagram.com/snappify" target="_blank" rel="noreferrer">
                        <InstagramIcon className="size-5 hover:text-pink-500" />
                    </a>
                    <a href="https://linkedin.com/company/snappify" target="_blank" rel="noreferrer">
                        <LinkedinIcon className="size-5 hover:text-pink-500" />
                    </a>
                    <a href="https://x.com/snappify" target="_blank" rel="noreferrer">
                        <TwitterIcon className="size-5 hover:text-pink-500" />
                    </a>
                    <a href="https://youtube.com/@snappify" target="_blank" rel="noreferrer">
                        <YoutubeIcon className="size-6 hover:text-pink-500" />
                    </a>
                </div>
                <p className="mt-3 text-center">&copy; {new Date().getFullYear()} Snappify. All rights reserved.</p>
            </motion.div>
        </footer>
    );
}