    ...
    <div key={gen.id} className="mb-4 break-inside-avoid">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
            ...
            <div className={`${gen?.aspectRatio==="9:16"? 'aspect-9/16':'aspect-video'} relative overflow-hidden`}>
                ...
                {!forCommunity && (
                    <div onMouseDownCapture={() => setMenuOpen(true)} onMouseLeave={() => setMenuOpen(false)} className="absolute top-3 right-3 sm:opacity-0 group-hover:opacity-100 transition flex items-center gap-2">
                        ...
                        <div className="flex flex-col items-end w-32 text-sm">
                            <ul className={`text-xs ${menuOpen? 'block': 'hidden'} overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10`}>
                                {gen.generatedImage && <a href="#" download={true} className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"><ImageIcon size={14}/>Download Image</a>}
                                {gen.generatedVideo && <a href="#" download={true} className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"><PlaySquareIcon size={14}/>Download Video</a>}
                                {(gen.generatedVideo || gen.generatedImage) && 
                                <button onClick={() => navigator.share({url: gen.generatedVideo || gen.generatedImage, title:gen.productName, text:gen.productDescription})} className="w-full flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"><Share2Icon size={14}/>Share</button>}
            ...
            <div className="p-4">
                ...
                {!forCommunity && (
                    <div className="mt-4 grid grid-cols-2 gap-3">
                        <button onClick={() => {navigate(`/result/${gen.id}`); scrollTo(0,0)}} className="flex items-center gap-2 border border-pink-900 hover:bg-pink-950/50 transition rounded-full px-6 h-11">View Details</button>
                        <button onClick={() => togglePublish(gen.id)} className="bg-pink-600 hover:bg-pink-700 text-white rounded-full px-7 h-11">{gen.isPublished? "Unpublish":"Publish"}</button>
                    </div>
                )}
            </div>
